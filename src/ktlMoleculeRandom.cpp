...
std::vector<double> ktlMolecule::checkOverlapWithRad(double &wRad){
  std::vector<double> overlappedSecs;
  distSets.clear();
  double minimumintraMolecularDistance = 10000.0;
  minimumintraMolecularDistanceMean  =0.0;
  int chainIndex1=0;
  int chainIndex2=0;
  for(int i=0;i<coords.size();i++){
    bool triggered =false;
    if(i>chainList[chainIndex1].second){
      ...
      chainIndex1++;
      //std::cout<<"eh ? "<<minimumintraMolecularDistance<<"\n";
      minimumintraMolecularDistanceMean =  minimumintraMolecularDistanceMean+ minimumintraMolecularDistance;
      minimumintraMolecularDistance = 10000.0;
    }
	...
	for(int l=0;l<coords[j].size();l++){
	  ...
	  if(chainIndex1!=chainIndex2){
	    if(dist<minimumintraMolecularDistance){
	      minimumintraMolecularDistance=dist;
	    }
	  }
  ...
  //std::cout<<"final distance "<<minimumintraMolecularDistance<<"\n";
  if(chainList.size()==1){
    minimumintraMolecularDistanceMean =  minimumintraMolecularDistanceMean+ minimumintraMolecularDistance;
  }else{
    minimumintraMolecularDistanceMean =  minimumintraMolecularDistanceMean/double(chainList.size()-1);
  }
  return overlappedSecs;
}
